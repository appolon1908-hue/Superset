...
|---|---|---|---|
| `GET` | `/health` | application and metadata health | authenticated edge/read-only |
| `GET` | `/api/v1/security/csrf_token/` | CSRF token for authenticated API use | authenticated session |
| `GET` | `/swagger/v1` | native API description | authenticated/controlled |
...
Expected unauthenticated behavior may be OIDC redirect, `401`, or `403`. Unexpected `404`, unhandled `5xx`, unrestricted SQL, or write-capable production credentials block certification.
...
GET_/swagger/v1_ROUTE_EXISTS=PASS
DASHBOARD_API=PASS
CHART_API=PASS
DATASET_API=PASS
...
Use synthetic read-only RLS fixtures and certified staging datasets. Do not query customer or financial production data merely to prove routing.

## Repository-first remediation

Preserve the previous healthy Superset workload on failure. Fix configuration, role mapping, RLS, query, or metadata defects here with regression tests; commit/push; exact-head CI/review; protected merge; signed immutable rebuild; BOM update; then retry. Never patch a role or datasource only in the live UI.
